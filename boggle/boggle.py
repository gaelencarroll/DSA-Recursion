...
def find(board, word):
    ...
    print("%-6s%d,%d  %-3s%-8s%-30s" %
          ('out', 'x', 'y', 'board', 'word', 'seen'))

    for y in range(0, 5):
        for x in range(0, 5):
            if find_from(board, word, x, y, seen=set()):
                return True
...
def find_from(board, word, x, y, seen):
    if (y, x) in seen:
        print("%-6s%d,%d  %-3s%-8s%-30s" %
              ('SEEN', x, y, board[y][x], word, seen))
        return False

    if word[0] != board[y][x]:
        print("%-6s%d,%d  %-3s%-8s%-30s" %
              ('WRONG', x, y, board[y][x], word, seen))
        return False

    if len(word) == 1:
        print("%-6s%d,%d  %-3s%-8s%-30s" %
              ('WIN', x, y, board[y][x], word, seen))
        return True
    print("%-6s%d,%d  %-3s%-8s%-30s" % ('OK', x, y, board[y][x], word, seen))
