...
function find(board, word) {
  ...
  for(let y = 0; y < 5; y++){
    for(let x = 0; x < 5; x++){
      if(findFrom(board, word, x, y, new Set())){
        return true;
      }
    }
  }
...
function findFrom(board, word, x, y, seen){
  if(word.length === 1){
    return true;
  }
  if(board[y][x] != word[0]){
  ...
  if(x > 0 && findFrom(board, word.slice(1), x-1, y, seen)){
    return true;
  }
  if(y < 4 && findFrom(board, word.slice(1), x+1, y, seen)){
    return true;
  }
  if(y > 0 && findFrom(board, word.slice(1), x, y-1, seen)){
    return true;
  }
  if(y < 4 && findFrom(board, word.slice(1), x, y+1, seen)){
    return true;
  }
  return false;
}
